  ...
  static async createShopApprovalNotification(shop, status, adminUser, rejectionReason = null) {
    try {
      ...
      if (status === "pending") {
        ...
        // For approved/rejected notifications, we need a valid recipient
        // Since shops don't have user accounts, we'll notify the approving admin instead
        // and send email separately to the shop owner
        if (adminUser && adminUser._id) {
          await Notification.create({
            recipient: adminUser._id,
            recipientType: "admin",
            type: type,
            title: `Shop ${status === 'approved' ? 'Approved' : 'Rejected'}: ${shop.name}`,
            message: message,
            data: {
              shopId: shop._id,
              shopName: shop.name,
            ...
            actionUrl: status === "approved" ? `/admin-sellers` : `/admin-pending-sellers`,
          });
        }
        // Note: Email notification to shop owner should be handled separately
        console.log(`Shop ${status} notification: ${shop.email} - ${message}`);
      }
    } catch (error) {
