...
const { isAuthenticated, isSeller, isSellerOrStoreManager, isAdmin, requirePermission, requireAnyPermission } = require("../middleware/auth");
...
router.post(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      const { cart, shippingAddress, user, totalPrice, paymentInfo, subTotalPrice, shippingPrice, discountPrice, tax, deliveryMethod, deliveryDistrict, deliveryFeeAmount, deliveryProviderType, pickupDetails } = req.body;
      ...
      for (const [shopId, items] of shopItemsMap) {
        ...
        const order = await Order.create({
          ...
          shopId: shopId,
          shopName: shopName,
          // District-based delivery fields (Mall of Cayman SRS)
          deliveryMethod: deliveryMethod || "COLLECT",
          deliveryDistrict: deliveryDistrict || null,
          deliveryFeeAmount: deliveryFeeAmount || 0,
          deliveryProviderType: deliveryProviderType || "VENDOR",
          deliveryChoiceTimestamp: deliveryMethod ? new Date() : null,
        ...
        if (shopId !== 'admin') {
          ...
          // Also notify the store manager if one is assigned (Store Manager SRS)
          try {
            ...
            if (storeManagerService && storeManagerService.assignedManager?.email) {
              const currencyFormatter = await createCurrencyFormatter();
              const manager = storeManagerService.assignedManager;
              
              // Send email notification to store manager
              await sendMail({
                email: manager.email,
                subject: `[Store Manager] New Order ${getOrderNumber(order)} - Action Required`,
                message: `Hello ${manager.name},\n\nA new order has been placed for the store you manage.\n\nOrder Details:\n- Order Number: ${getOrderNumber(order)}\n- Items: ${order.cart.length} products\n- Total: ${currencyFormatter.format(order.totalPrice)}\n- Payment: ${isCODOrder ? 'Cash on Delivery' : 'Prepaid'}\n\nPlease coordinate with the store owner regarding payment confirmation before fulfilling the order.\n\nManage this order at: ${process.env.FRONTEND_URL || 'http://localhost:3000'}/store-manager/order/${order._id}\n\nBest regards,\nMall of Cayman Team`
              });
              
              console.log(`📧 Store Manager notification sent to ${manager.email} for order ${getOrderNumber(order)}`);
            }
          } catch (smNotifyError) {
            console.log('Store Manager notification skipped:', smNotifyError.message);
            // Non-fatal - don't block order creation if SM notification fails
          }
        }
      }
...
// Get order details for seller or store manager
router.get(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      const orderBelongsToSeller = order.cart && order.cart.some(item =>
        item.shopId && item.shopId.toString() === sellerId
      );

      if (!orderBelongsToSeller) {
        return next(new ErrorHandler("This order does not belong to your shop", 403));
      }
...
// update order status for seller or store manager    ---------------(product)
router.put(
...
// Download invoice PDF (Seller or Store Manager)
router.get(
