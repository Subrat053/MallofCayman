...
const ProductCard = ({ data, isEvent, isCompact = false }) => {
  ...
  // Fetch vendor delivery configuration
  const fetchVendorDeliveryConfig = useCallback(async () => {
    try {
      const response = await axios.get(
        `${server}/vendor-delivery/config/${data.shopId}`
      );
      if (response.data.success) {
        setVendorDeliveryConfig(response.data.config);
      }
    } catch (error) {
      // Vendor may not have configured delivery yet
      console.log("Could not fetch vendor delivery config:", error);
    }
  }, [data?.shopId]);
  ...
  }, [data?.shopId, fetchVendorDeliveryConfig]);
  ...
  const buyNowHandler = async () => {
    ...
    const orderData = {
      ...
      shipping: 0, // Delivery fee will be calculated at checkout
      discountPrice: 0,
  ...
  // Check delivery options availability
  const getDeliveryBadge = () => {
    if (!vendorDeliveryConfig) return null;

    const hasDelivery =
      vendorDeliveryConfig.deliveryEnabled &&
      vendorDeliveryConfig.districtFees?.some((df) => df.isAvailable);
    const hasPickup = vendorDeliveryConfig.pickupEnabled !== false;

    // Check if free delivery is available for this product price
    const hasFreeDelivery =
      vendorDeliveryConfig.freeDeliveryThreshold &&
      data.discountPrice >= vendorDeliveryConfig.freeDeliveryThreshold;

    if (hasFreeDelivery && hasDelivery) {
      return { text: "🚚 Free Delivery", color: "text-green-600" };
    } else if (hasDelivery && hasPickup) {
      return { text: "🚚 Delivery / 🏪 Pickup", color: "text-blue-600" };
    } else if (hasDelivery) {
      return { text: "🚚 Delivery Available", color: "text-blue-600" };
    } else if (hasPickup) {
      return { text: "🏪 Pickup Only", color: "text-orange-600" };
    }
    return null;
  };
      ...
      <div className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden border border-gray-200 h-full flex flex-col w-full">
        ...
        <div className="p-3 flex flex-col flex-1">
          ...
          <div className="flex items-center justify-between mt-1 text-xs">
            ...
            {getDeliveryBadge() && (
              <span className={`${getDeliveryBadge().color} text-[11px]`}>
                {getDeliveryBadge().text}
              </span>
            )}
          </div>
