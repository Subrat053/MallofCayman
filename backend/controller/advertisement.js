...
exports.calculateAdvertisementPrice = catchAsyncErrors(async (req, res, next) => {
  const { adType, duration, shopId } = req.body;
  ...
  const isFreeMode = plan?.isFree || false;
  
  // Check if the shop is ad fee exempt (in-house store or special partner)
  let isShopAdFeeExempt = false;
  let adFeeExemptReason = "";
  
  // Use shopId from request or from authenticated seller
  const checkShopId = shopId || req.seller?._id;
  if (checkShopId) {
    const shop = await Shop.findById(checkShopId).select('adFeeExempt adFeeExemptReason isInHouseStore');
    if (shop && shop.adFeeExempt) {
      isShopAdFeeExempt = true;
      adFeeExemptReason = shop.adFeeExemptReason || (shop.isInHouseStore ? "In-House Store" : "Special Partner");
    }
  }
  
  const isFree = isFreeMode || isShopAdFeeExempt;
  ...
  res.status(200).json({
    ...
    pricing: {
      ...
      totalPrice,
      isFree,
      isFreeMode, // Global free mode for this ad type
      isShopAdFeeExempt, // Shop-specific exemption
      adFeeExemptReason,
    },
  });
});
...
exports.createAdvertisement = catchAsyncErrors(async (req, res, next) => {
  ...
  // Check if shop is ad fee exempt (in-house store or special partner)
  const isShopAdFeeExempt = shop.adFeeExempt || false;
  const adFeeExemptReason = shop.adFeeExemptReason || (shop.isInHouseStore ? "In-House Store" : "");
  ...
  // Determine if this ad should be free (free mode OR shop is ad fee exempt)
  const isFree = isFreeMode || isShopAdFeeExempt;
  ...
  // In-house stores or ad fee exempt shops also get auto-approval
  const shouldAutoApprove = hasAdPreApproval || isShopAdFeeExempt;
  
  if (isFree) {
    // Free mode or shop ad fee exempt - skip payment
    advertisement.paymentStatus = 'completed';
    advertisement.totalPrice = 0;
    advertisement.paymentMethod = isShopAdFeeExempt ? 'ad_fee_exempt' : 'free_testing';
    
    // If seller has Ad Pre-Approval (Gold plan) or is ad fee exempt, auto-approve
    if (shouldAutoApprove) {
      advertisement.status = 'active';
      advertisement.approvedAt = new Date();
      advertisement.approvalNote = isShopAdFeeExempt 
        ? `Auto-approved (Ad Fee Exempt: ${adFeeExemptReason})`
        : 'Auto-approved (Gold Plan - Ad Pre-Approval feature)';
    } else {
  ...
  let message = "";
  if (isFree && shouldAutoApprove) {
    message = isShopAdFeeExempt 
      ? "Advertisement created successfully (Ad Fee Exempt + Auto-Approved). Your ad is now LIVE!"
      : "Advertisement created successfully (FREE MODE + Auto-Approved). Your ad is now LIVE!";
  } else if (isFree) {
  ...
  res.status(201).json({
    ...
    isFree,
    isShopAdFeeExempt,
    autoApproved: isFree && shouldAutoApprove,
  });
});
