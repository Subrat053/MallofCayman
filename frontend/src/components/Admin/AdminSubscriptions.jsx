    ...
    <div className="w-full bg-white rounded-lg shadow-sm p-3 sm:p-6 overflow-hidden">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 mb-4 sm:mb-6">
        ...
        <div className="bg-white border border-red-200 rounded-lg p-3 sm:p-4">
          <p className="text-xs sm:text-sm text-gray-600 mb-1">No Sub</p>
          <p className="text-xl sm:text-3xl font-bold text-red-600">
            {shopsWithoutSub.length}
          </p>
        </div>
      </div>
      {/* Search Bar */}
      <div className="mb-3 sm:mb-4">
        <input
          ...
          className="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
        />
      </div>
      {/* Filter Tabs */}
      <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4 sm:mb-6">
        {["all", "active", "expired", "cancelled", "no-subscription"].map(
          (status) => (
            <button
              key={status}
              onClick={() => setFilter(status)}
              className={`px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium capitalize ${
                filter === status
                  ? "bg-primary-600 text-white"
                  : "bg-gray-100 text-gray-600 hover:bg-gray-200"
              }`}
            >
              {status === "no-subscription" ? "No Sub" : status}
            </button>
          )
        )}
      </div>
      ...
      <div className="lg:hidden space-y-3">
        {/* Shops without subscription - Mobile */}
        {(filter === "all" || filter === "no-subscription") &&
          shopsWithoutSub
            .filter(
              (shop) =>
                !searchTerm ||
            ...
            .map((shop) => (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700 ml-2 whitespace-nowrap">
                    No Sub
                  </span>
                </div>
                <button
                  onClick={() => openAssignModal(shop)}
                  className="w-full mt-2 px-3 py-2 bg-green-600 text-white rounded-lg text-sm font-medium flex items-center justify-center gap-1"
                >
                  <HiPlus size={14} />
                  Assign Subscription
                </button>
        ...
        {/* Shops with subscription - Mobile */}
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                ...
                <div>
                  ...
                  <span className="font-medium">${sub.finalPrice}</span>
                </div>
                <div>
              ...
              {sub.status === "active" && (
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 px-3 py-1.5 bg-blue-50 text-blue-600 rounded-lg text-xs font-medium flex items-center justify-center gap-1"
                  >
                    <FiEdit2 size={12} />
                    Change
                  </button>
                  <button
                    ...
                    className="flex-1 px-3 py-1.5 bg-red-50 text-red-600 rounded-lg text-xs font-medium disabled:opacity-50"
                  >
      ...
      {filteredSubscriptions.length === 0 && shopsWithoutSub.length === 0 && (
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-semibold text-gray-900">
                    {selectedSubscription.shop?.name}
                  </p>
                  <p className="text-sm text-gray-500">
                    {selectedSubscription.shop?.email}
                  </p>
                </div>
                <span
                  className={`px-3 py-1 rounded-full text-sm font-medium capitalize ${
                    selectedSubscription.plan === "gold"
                      ? "bg-yellow-100 text-yellow-800"
                      : selectedSubscription.plan === "silver"
                      ? "bg-gray-200 text-gray-800"
                      : selectedSubscription.plan === "bronze"
                      ? "bg-orange-100 text-orange-800"
                      : "bg-blue-100 text-blue-800"
                  }`}
                >
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-gray-500 mt-1">
                            ${planData.monthlyPrice}/month • Up to{" "}
                            {planData.maxProducts} products
                          </p>
                        </div>
                        {newPlan === planKey && (
            ...
            <div className="flex space-x-3">
              ...
              >
                {isChanging ? (
                  ...
                  <>
                    {getChangeType(selectedSubscription.plan, newPlan) ===
                      "upgrade" && <HiArrowUp />}
                    {getChangeType(selectedSubscription.plan, newPlan) ===
                      "downgrade" && <HiArrowDown />}
                    Confirm Change
                  </>
                )}
              </button>
          ...
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-4">
              ...
              <input
                ...
                onChange={(e) =>
                  setAssignDuration(parseInt(e.target.value) || 1)
                }
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
              />
            </div>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    No payment required from seller
                  </p>
                </div>
              </label>
            </div>
                  ...
                  {assignIsFree ? (
                    ...
                    <>
                      ...
                      <span className="font-semibold text-gray-900">
                        $
                        {(
                          availablePlans[assignPlan].monthlyPrice *
                          assignDuration
                        ).toFixed(2)}
                      </span>
                      <span className="text-gray-500">
                        {" "}
                        for {assignDuration} month(s)
                      </span>
                    </>
                  )}
                </p>
...
const StatCard = ({ title, value, color }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-3 sm:p-4">
      ...
      <p className={`text-xl sm:text-3xl font-bold ${colors[color]}`}>
        {value}
      </p>
    </div>
  );
