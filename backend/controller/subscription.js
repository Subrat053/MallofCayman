...
// Get available subscription plans (for sellers - only active paid plans)
router.get(
  "/get-plans",
  catchAsyncErrors(async (req, res, next) => {
    // Get active plans from database, excluding the free plan (free is default for sellers)
    const plans = await SubscriptionPlan.find({ 
      isActive: true,
      planKey: { $ne: 'free' } // Exclude free plan from purchase options
    }).sort({ sortOrder: 1 });
    const activePlans = {};
    for (const plan of plans) {
...
router.get(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      const Subscription = require('../model/subscription');
      // Only return active subscriptions - pending subscriptions should not be shown as active
...
router.post(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      if (existingActive) {
        return next(new ErrorHandler('You already have an active subscription. Please cancel it first to subscribe to a new plan.', 400));
      }

      // Delete any old pending subscriptions (cleanup from abandoned payment attempts)
...
router.post(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      let userMessage = 'Failed to activate subscription. Please try again.';
      ...
      try {
        await Subscription.findByIdAndDelete(req.body.subscriptionId);
      } catch (deleteError) {
        console.error('Failed to delete pending subscription:', deleteError);
      }
...
router.put(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      // Update shop subscription status - set to 'free' instead of null
