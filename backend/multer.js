...
// File filter for license documents (images and PDFs)
const licenseFilter = (req, file, cb) => {
  // Allow images and PDFs for license documents
  if (file.mimetype.startsWith('image/') || file.mimetype === 'application/pdf') {
    cb(null, true);
  } else {
    cb(new Error('Only images (JPG, PNG) and PDF files are allowed for license documents!'), false);
  }
};
...
// Shop registration upload - avatar + trade licenses
exports.uploadShopRegistration = multer({
  storage: storage,
  ...
  { name: 'tradeLicenses', maxCount: 5 } // Trade/Business licenses (up to 5)
]);
